...
The `biocontainers` module unlocks all bioinformatics software. You must load it before any tool module becomes visible. Use `module --force purge` (not just `module purge`) to remove **sticky modules** like `xalt` that use a newer `glibc` and can conflict with containerized tools.

After loading, run the tool as usual:

```bash frame="none"
samtools --version
```

Behind the scenes, RCAC provides wrapper scripts so that containerized tools behave like native commands. When you type `samtools`, the wrapper runs `apptainer exec <container.sif> samtools` for you.

### Understanding the wrapper

Because the tools are containerized, `which` and `type` will show the **wrapper script**, not the actual executable inside the container:

```bash frame="none"
which samtools
# /apps/biocontainers/bin/samtools  (wrapper script)
type samtools
# samtools is /apps/biocontainers/bin/samtools
```

For most use cases this does not matter — the wrapper handles everything transparently. However, if a pipeline or workflow checks the executable path (e.g., `which samtools` to verify the installation), you may need to either:

- Write a custom wrapper script that handles the path check
...
You can use this to run containers directly with `apptainer exec` when you need more control (e.g., custom bind mounts, GPU flags, or piping between containerized tools):

```bash frame="none"
apptainer exec ${BIOC_IMAGE_DIR}/samtools_1.21.sif samtools view -h input.bam
```
...
- **Open OnDemand**: [ondemand.rcac.purdue.edu](https://ondemand.rcac.purdue.edu)
